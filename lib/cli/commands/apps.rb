...
#      return display JSON.pretty_generate(apps) if @options[:json]
    ...
    def do_push(appname=nil)
      ...
      default_runtime = VMC::Cli::Framework.detect_default_runtime(@application,framework.name) if not runtime
      runtime = detect_runtime(default_runtime, !no_prompt) if default_runtime
