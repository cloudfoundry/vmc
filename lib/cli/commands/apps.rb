...
#      return display JSON.pretty_generate(apps) if @options[:json]
    ...
    def do_push(appname=nil)
      ...
      if not runtime
        default_runtime = VMC::Cli::Framework.detect_default_runtime(@application,framework.name)
        runtime = detect_runtime(default_runtime, !no_prompt) if VMC::Cli::Framework.prompt_for_runtime? framework.name
      end
      command = ask("Start Command") if !command && VMC::Cli::Framework.require_start_command?(framework.name)

      default_url = "None"
      default_url = "#{appname}.#{VMC::Cli::Config.suggest_url}" if VMC::Cli::Framework.require_url?(framework.name)

      unless no_prompt || url || !VMC::Cli::Framework.require_url?(framework.name)
        url = ask(
      ...
      if memswitch
        ...
        mem = ask("Memory Reservation",
                  :default => VMC::Cli::Framework.detect_default_memory(framework.name,runtime),
                  :choices => mem_choices)
      else
        mem = VMC::Cli::Framework.detect_default_memory(framework.name,runtime)
      end
