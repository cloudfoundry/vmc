...
module VMC::Cli::ManifestHelper
  ...
  def configure_app(many=false)
    name = set ask("Application Name", :default => manifest("name")), "name"

    url = ask("Application Deployed URL", :default => manifest("url"))
    ...
    set ask(
      "Memory reservation",
      :default => manifest("mem") || manifest("framework", "info", "mem"),
      :choices => ["128M", "256M", "512M", "1G", "2G"]
    ), "mem"

    set ask("How many instances?", :default => manifest("instances")), "instances"
  ...
  def configure_service(vendor)
    ...
    set ask(
      "What do you want to do if it already exists?",
      :default => "keep",
      :choices => ["delete", "keep"]
    ), "services", name, "if_exists"
  end
end
