...
module VMC::Cli::ManifestHelper
  ...
  def configure_app(many=false)
    ...
    if not default_runtime
      default_runtime = VMC::Cli::Framework.detect_default_runtime(@application,manifest("framework", "name"))
      #Don't allow user to choose if default runtime comes back nil.
      #Runtime will be chosen on the server side based on framework
      set(detect_runtime(default_runtime), "runtime") if VMC::Cli::Framework.prompt_for_runtime? manifest("framework", "name")
    end
