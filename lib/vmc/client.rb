...
class VMC::Client
  ...
  def check_resources(resources)
    check_login_status
    _, body, _ = json_post VMC::RESOURCES_PATH, resources
    json_parse body
  end
  ...
  def logged_in?
    ...
    return false unless descr[:user] && descr[:usage]
    @user = descr[:user]
    true
  end
  ...
  def json_get(url)
    _, body, _ = http_get url, 'application/json'
    json_parse body
  ...
  def request(method, path, content_type = nil, payload = nil, headers = {})
    ...
    req = {
      :method => method, :url => [@target, path].join('/'),
