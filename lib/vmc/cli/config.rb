  ...
  class Config
    ...
    class << self
      ...
      def target_url
        ...
        @target_url = if File.exists? target_file
          lock_and_read(target_file).strip
        else
          DEFAULT_TARGET
        end
      ...
      def instances
        ...
        return unless File.exists? instances_file
      ...
      def clients
        ...
        @clients = if File.exists? clients
          user = YAML.load_file clients
          deep_merge stock, user
        else
          stock
        end
      end
