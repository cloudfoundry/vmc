...
describe 'VMC::Cli::ConsoleHelper' do
  ...
  it 'should output a message when no app instances found' do
    ...
    @msg.should == "No running instances for [foo]"
  end

  it 'should output a message when app does not have console access' do
    @client.should_receive(:app_instances).with("foo").and_return({:instances=>[{}]})
    console_connection_info('foo')
    @msg.should == ("Console access not supported for [foo]. Try running 'vmc files foo app/cf-rails-console'." +
      " If the directory is not found, you need to redeploy your app.")
  end
